...
export default function LandingPageLocalized({ variant }: { variant: 'a' | 'b' }) {
  ...
  const copy = byLanguage(language, {
    pl: {
      ...
      pricing: { title: '🚀 Wybierz plan, ktory sprzedaje wynik szybciej niz kosztuje', plans: [['Free', '$0', 'Zacznij bez ryzyka', 'Przetestuj za darmo — 5 generacji dziennie, pelny Assets Studio FREE i zero karty kredytowej'], ['Pro', '$19', 'Szybki start', '70 generacji dziennie — idealny plan dla solo tworcow, coachow i mikro-agencji'], ['Premium Plus', '$49', '🔥 Najlepsza wartosc', '220 generacji dziennie — publish, content brain i revenue AI w jednym pakiecie'], ['Expert', '$99', 'Dla skali', '500 generacji dziennie — priorytet, najwyzsze limity i pelny stack wzrostu']], choose: 'Zacznij teraz', start: 'Uruchom Premium Plus ->', topup: '💡 Potrzebujesz skoku bez zmiany planu? Dokup jednorazowo: 25 generacji za $9, 75 za $24 lub 200 za $49 — bez umowy i bez abonamentu.' },
      faq: { title: 'Najczestsze obiekcje przed zakupem', items: [['Czy moge zrezygnowac w dowolnym momencie?', 'Tak. Subskrypcje sa miesieczne i mozesz je anulowac bez rozmowy ze sprzedawca i bez ukrytych kar.'], ['Skad mam wiedziec, ze to mi sie zwroci?', 'Dlatego Premium Plus jest planem domyslnym: laczy content, publish i revenue AI, wiec szybciej zamienia publikacje w realny wynik.'], ['A jesli potrzebuje skali tylko czasowo?', 'Nie musisz od razu zmieniac planu. Masz top-upy jednorazowe 25 / 75 / 200 generacji.'], ['Czy moje platnosci i dane sa bezpieczne?', 'Tak. Checkout idzie przez Stripe, sesje wygasaja automatycznie, webhook weryfikuje kwote i walute, a panel admin ma 2FA.']] },
    ...
    en: {
      ...
      pricing: { title: '🚀 Choose the plan that pays back faster than it costs', plans: [['Free', '$0', 'Try risk-free', 'Start free — 5 generations per day, full FREE Assets Studio, no credit card required'], ['Pro', '$19', 'Fast start', '70 daily generations — perfect for solo creators, consultants, and lean agencies'], ['Premium Plus', '$49', '🔥 Best value', '220 daily generations — publish, content brain, and revenue AI in one growth stack'], ['Expert', '$99', 'Built for scale', '500 daily generations — priority processing, highest limits, and the full growth system']], choose: 'Get started', start: 'Launch Premium Plus ->', topup: '💡 Need a burst without upgrading? Buy one-time: 25 credits for $9, 75 for $24, or 200 for $49 — no contract, no subscription.' },
      faq: { title: 'Most common buying objections', items: [['Can I cancel anytime?', 'Yes. All subscriptions are monthly and can be cancelled without sales calls, lock-ins, or hidden penalties.'], ['How do I know this will pay back?', 'That is exactly why Premium Plus is highlighted: it combines content, publishing, and revenue AI, so it turns activity into measurable output faster.'], ['What if I only need scale temporarily?', 'You do not need to upgrade immediately. One-time top-ups give you 25 / 75 / 200 extra generations on demand.'], ['Are my payments and data secure?', 'Yes. Checkout runs through Stripe, sessions expire automatically, the webhook verifies amount and currency, and the admin stack is protected by 2FA.']] },
    ...
    es: {
      ...
      pricing: { title: '🚀 Elige el plan que devuelve valor antes de costarte de verdad', plans: [['Free', '$0', 'Prueba sin riesgo', 'Empieza gratis — 5 generaciones al dia, Assets Studio FREE completo y sin tarjeta'], ['Pro', '$19', 'Inicio rapido', '70 generaciones diarias — ideal para creadores, consultores y micro-agencias'], ['Premium Plus', '$49', '🔥 Mejor valor', '220 generaciones diarias — publish, content brain y revenue AI en un solo stack'], ['Expert', '$99', 'Hecho para escala', '500 generaciones diarias — prioridad, maximos limites y el sistema completo']], choose: 'Empezar ahora', start: 'Lanzar Premium Plus ->', topup: '💡 Necesitas un empuje sin subir de plan? Compra puntual: 25 creditos por $9, 75 por $24 o 200 por $49 — sin contrato y sin suscripcion.' },
      faq: { title: 'Objeciones más comunes antes de comprar', items: [['¿Puedo cancelar cuando quiera?', 'Sí. Todas las suscripciones son mensuales y puedes cancelarlas sin llamadas comerciales ni penalizaciones ocultas.'], ['¿Cómo sé si esto me dará retorno?', 'Por eso Premium Plus está destacado: une contenido, publicación y revenue AI para convertir actividad en resultado medible más rápido.'], ['¿Y si solo necesito escala de forma temporal?', 'No necesitas subir de plan enseguida. Tienes top-ups puntuales de 25 / 75 / 200 generaciones.'], ['¿Mis pagos y datos están seguros?', 'Sí. El checkout usa Stripe, las sesiones expiran automáticamente, el webhook valida importe y moneda, y el panel admin está protegido con 2FA.']] },
  ...
  async function runDemo() {
    ...
    try {
      ...
      const customerEmail = user?.email || window.localStorage.getItem('usinf_signup_email') || '';
      const res = await fetch('/api/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ topic, niche, plan, platform: 'all', language, attachmentContext, customerEmail }) });
  ...
  return (
    <main className="page-bg">
      ...
      <section className="section" id="pricing"><div className="section-header"><p className="section-label">{copy.sectionLabels[3]}</p><h2>{copy.pricing.title}</h2><p style={{ color: 'var(--muted)', maxWidth: 860, marginTop: 10 }}>{language === 'pl' ? 'Aktywacja natychmiast po Stripe Checkout. Rezygnujesz kiedy chcesz. Premium Plus jest domyslnym wyborem dla najlepszej konwersji ceny do wyniku.' : language === 'es' ? 'Activacion inmediata tras Stripe Checkout. Cancelas cuando quieras. Premium Plus es la eleccion por defecto para la mejor relacion entre precio y resultado.' : 'Instant activation after Stripe Checkout. Cancel anytime. Premium Plus is the default recommendation for the best price-to-growth ratio.'}</p></div><div className="pricing-grid">{copy.pricing.plans.map(([name, price, badge, limit], index, all) => { const planKey = (['free', 'pro', 'premium_plus', 'expert'][index] || 'free') as 'free' | 'pro' | 'premium_plus' | 'expert'; return <article key={String(name)} className={`price-card${index === 2 ? ' best' : ''}`}><div className="price-badge">{badge}</div><h3 style={{ fontSize: 20, marginBottom: 10 }}>{name}</h3><div className="price-amount">{price}<small>/mo</small></div><div className="price-limit">{limit}</div><button className={`btn ${index === 2 ? 'btn-primary' : 'btn-ghost'} btn-full`} style={{ marginTop: 20 }} onClick={() => openCheckout('subscription', planKey)} disabled={checkoutLoadingKey === `subscription:${planKey}`}>{checkoutLoadingKey === `subscription:${planKey}` ? (language === 'pl' ? 'Otwieranie...' : language === 'es' ? 'Abriendo...' : 'Opening...') : (index === 2 ? copy.pricing.start : copy.pricing.choose)}</button></article>; })}</div><p style={{ textAlign: 'center', color: 'var(--muted)', marginTop: 14 }}>{copy.pricing.topup}</p></section>

      <section className="section" id="faq"><div className="section-header"><p className="section-label">FAQ</p><h2>{copy.faq.title}</h2></div><div className="feature-grid">{copy.faq.items.map(([question, answer]) => <article key={question} className="feature-card"><h3 style={{ marginBottom: 10 }}>{question}</h3><p>{answer}</p></article>)}</div></section>
