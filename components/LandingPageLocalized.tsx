...
export default function LandingPageLocalized({ variant }: { variant: 'a' | 'b' }) {
  ...
  const copy = byLanguage(language, {
    pl: {
      ...
      heroA: { eyebrow: 'USInf.com | AI Operating System dla tworcow globalnych', title1: 'Publikuj szybciej od konkurencji.', title2: 'Zarabiaj na przewidywalnym wzroscie.', subtitle: 'USInf.com laczy stratega, content factory, publish engine i AI coacha w jednym workflow. Jeden material -> wiele platform -> realny wzrost.', main: 'Uruchom USInf Growth OS', alt: 'Porownaj plan wzrostu' },
      heroB: { eyebrow: 'USInf.com | Performance stack dla creator economy', title1: 'One Click Publish + AI Content Brain.', title2: 'Wyniki, nie chaos contentowy.', subtitle: 'Zamien kazdy film w pipeline wzrostu: produkcja, publikacja, analiza i automatyczne rekomendacje godzin i tematow pod najlepszy ROI.', main: 'Uruchom strategie testowa', alt: 'Zobacz case study ROI' },
      ...
      featuresTitle: 'Komplet funkcji dla profesjonalnych tworcow',
      features: [
      ...
      pricing: { title: 'Konkurencyjne ceny + kontrola kosztow API', plans: [['Free', '$0', 'Start', '5 generacji/dzien'], ['Pro', '$24', 'Najlepszy start', '60 generacji/dzien'], ['Premium Plus', '$69', 'One Click Publish', '180 generacji/dzien']], choose: 'Wybierz plan', start: 'Zacznij teraz ->', topup: 'Pakiety jednorazowe: +25/$9, +75/$19, +200/$39' },
      cta: { title: 'USInf.com: Twoj globalny system wzrostu creator-first.', text: 'Buduj audience i przychod szybciej dzieki hybrydowemu AI stackowi: wysoka jakosc tam gdzie ROI jest najwyzsze, lekki model tam gdzie liczy sie skala i koszt.', growth: 'Uruchom panel growth', admin: 'Zobacz panel admina' },
    ...
    en: {
      ...
      heroA: { eyebrow: 'USInf.com | AI Operating System for global creators', title1: 'Publish faster than the competition.', title2: 'Monetize predictable growth.', subtitle: 'USInf.com combines strategy, content factory, publish engine, and AI coach in one workflow. One asset -> many platforms -> real growth.', main: 'Launch USInf Growth OS', alt: 'Compare growth plans' },
      heroB: { eyebrow: 'USInf.com | Performance stack for creator economy', title1: 'One Click Publish + AI Content Brain.', title2: 'Results, not content chaos.', subtitle: 'Turn every video into a growth pipeline: production, publishing, analysis, and automatic recommendations for best hours and topics.', main: 'Start trial strategy', alt: 'See ROI case study' },
      ...
      featuresTitle: 'Complete feature set for professional creators',
      features: [
      ...
      pricing: { title: 'Competitive pricing + API safety control', plans: [['Free', '$0', 'Start', '5 generations/day'], ['Pro', '$24', 'Best start', '60 generations/day'], ['Premium Plus', '$69', 'One Click Publish', '180 generations/day']], choose: 'Choose plan', start: 'Start now ->', topup: 'One-time packs: +25/$9, +75/$19, +200/$39' },
      cta: { title: 'USInf.com: your creator-first global growth system.', text: 'Build audience and revenue faster with a hybrid AI stack: high quality where ROI matters most, lightweight models where scale and cost matter.', growth: 'Open growth panel', admin: 'Open admin panel' },
    ...
    es: {
      ...
      heroA: { eyebrow: 'USInf.com | Sistema operativo AI para creadores globales', title1: 'Publica mas rapido que la competencia.', title2: 'Monetiza crecimiento predecible.', subtitle: 'USInf.com une estrategia, content factory, publish engine y AI coach en un flujo unico. Un activo -> muchas plataformas -> crecimiento real.', main: 'Lanzar USInf Growth OS', alt: 'Comparar planes de crecimiento' },
      heroB: { eyebrow: 'USInf.com | Stack performance para creator economy', title1: 'One Click Publish + AI Content Brain.', title2: 'Resultados, no caos de contenido.', subtitle: 'Convierte cada video en un pipeline de crecimiento: produccion, publicacion, analisis y recomendaciones automaticas de horas y temas.', main: 'Iniciar estrategia de prueba', alt: 'Ver caso ROI' },
      ...
      featuresTitle: 'Conjunto completo de funciones para creadores profesionales',
      features: [
      ...
      pricing: { title: 'Precios competitivos + control de coste API', plans: [['Free', '$0', 'Inicio', '5 generaciones/dia'], ['Pro', '$24', 'Mejor inicio', '60 generaciones/dia'], ['Premium Plus', '$69', 'One Click Publish', '180 generaciones/dia']], choose: 'Elegir plan', start: 'Empezar ahora ->', topup: 'Packs unicos: +25/$9, +75/$19, +200/$39' },
      cta: { title: 'USInf.com: tu sistema global de crecimiento creator-first.', text: 'Construye audiencia e ingresos mas rapido con un stack AI hibrido: alta calidad donde el ROI importa mas, modelos ligeros donde importan escala y coste.', growth: 'Abrir panel growth', admin: 'Abrir panel admin' },
  ...
  return (
    <main className="page-bg">
      <nav className="nav">
        ...
        <div className="navlinks"><a href="#factory">{copy.nav.factory}</a><a href="#trends">{copy.nav.trends}</a><a href="/dashboard">{copy.nav.dashboard}</a><a href="#pricing">{copy.nav.pricing}</a></div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <LanguageSwitcher compact />
          <a href="/dashboard/account" className="btn btn-ghost btn-sm">{copy.nav.register}</a>
          <a href="/dashboard/account" className="btn btn-ghost btn-sm"><UserCircle2 size={13} /> {copy.nav.account}</a>
          {!isAdmin ? <a href="/dashboard/admin" className="btn btn-ghost btn-sm"><LogIn size={13} /> {copy.nav.login}</a> : <button className="btn btn-ghost btn-sm" onClick={logout}><LogOut size={13} /> {copy.nav.logout}</button>}
          <a href="/dashboard" className="btn btn-primary btn-sm">{copy.nav.enter}</a>
        </div>
      </nav>
      ...
      <section className="hero">
        <div>
          ...
          <div className="cta-row"><a href="/dashboard" className="btn btn-primary btn-lg">{hero.main} <ArrowRight size={18} /></a><a href="#pricing" className="btn btn-ghost btn-lg">{hero.alt}</a></div>
      ...
      <section className="section" id="pricing"><div className="section-header"><p className="section-label">{copy.sectionLabels[3]}</p><h2>{copy.pricing.title}</h2></div><div className="pricing-grid">{copy.pricing.plans.map(([name, price, badge, limit], index, all) => { const planKey = index === 0 ? 'free' : index === 1 ? 'pro' : 'premium_plus'; return <article key={String(name)} className={`price-card${index === 1 ? ' best' : ''}`}><div className="price-badge">{badge}</div><h3 style={{ fontSize: 20, marginBottom: 10 }}>{name}</h3><div className="price-amount">{price}<small>/mo</small></div><div className="price-limit">{limit}</div><button className={`btn ${index === 1 ? 'btn-primary' : 'btn-ghost'} btn-full`} style={{ marginTop: 20 }} onClick={() => openCheckout('subscription', planKey)} disabled={checkoutLoadingKey === `subscription:${planKey}`}>{checkoutLoadingKey === `subscription:${planKey}` ? (language === 'pl' ? 'Otwieranie...' : language === 'es' ? 'Abriendo...' : 'Opening...') : (index === 1 ? copy.pricing.start : copy.pricing.choose)}</button></article>; })}</div><p style={{ textAlign: 'center', color: 'var(--muted)', marginTop: 14 }}>{copy.pricing.topup}</p></section>

      <section className="section" id="topup"><div className="section-header"><p className="section-label">{language === 'pl' ? 'Zakup jednorazowy' : language === 'es' ? 'Compra unica' : 'One-time purchase'}</p><h2>{language === 'pl' ? 'Dokup generacje, gdy potrzebujesz skali' : language === 'es' ? 'Compra generaciones cuando necesites escala' : 'Buy extra generations when you need scale'}</h2></div><div className="pricing-grid">{Object.values(TOP_UP_PACKS).map((pack) => <article key={pack.id} className="price-card"><div className="price-badge">{pack.bonusLabel || (language === 'pl' ? 'Jednorazowe' : language === 'es' ? 'Unica vez' : 'One-time')}</div><h3 style={{ fontSize: 20, marginBottom: 10 }}>{pack.label}</h3><div className="price-amount">${pack.priceUsd}<small>one-time</small></div><div className="price-limit">{pack.generations} {language === 'pl' ? 'generacji' : language === 'es' ? 'generaciones' : 'generations'}</div><button className="btn btn-primary btn-full" style={{ marginTop: 20 }} onClick={() => openCheckout('topup', pack.id)} disabled={checkoutLoadingKey === `topup:${pack.id}`}>{checkoutLoadingKey === `topup:${pack.id}` ? (language === 'pl' ? 'Otwieranie...' : language === 'es' ? 'Abriendo...' : 'Opening...') : (language === 'pl' ? 'Kup pakiet' : language === 'es' ? 'Comprar paquete' : 'Buy pack')}</button></article>)}</div>{billingMessage ? <div className="alert alert-info" style={{ marginTop: 14, maxWidth: 920, marginLeft: 'auto', marginRight: 'auto' }}>{billingMessage}</div> : null}</section>

      <section className="section" style={{ paddingTop: 16 }}><div className="global-cta"><h2>{copy.cta.title}</h2><p>{copy.cta.text}</p><div className="cta-row" style={{ justifyContent: 'center', marginBottom: 0 }}><a href="/dashboard" className="btn btn-gradient btn-lg">{copy.cta.growth}</a><a href="/dashboard/admin" className="btn btn-ghost btn-lg">{copy.cta.admin}</a></div></div></section>
